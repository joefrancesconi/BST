...
Node *insert_random(Node *T, int v, int r)
{
  ...
  bool probability = (rand() % (T->size + 1)) == 0;
...
Node *order_n_teams(int n)
{
  ...
  for (int i=1; i<n; i++) {
    ...
    if (did_x_beat_y(i, select(T,0)->key)) // can we insert at beginning?
      ...
      int low = 0, high = T->size - 1, middle = 0;
      bool p = false;
      while (!p)
      {
        ...
        else if (did_x_beat_y(i, select(T, middle)->key))
          high = middle;
        else
          low = middle;
      }

      T = insert_random(T, i, middle);
